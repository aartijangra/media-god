...
const configuration = {
  ...
  SiteNameSize: .6, // Between 0 and +
  NumberOfVerticalLines: 15,
  NumberOfDots: 5000,
  colors: {
    CanvasBackgroundColor: '#141414',
    LettersColor: '#5183a7',
    LinesColors: ['#FFF', '#5183a7', '#7d7d7d'],
...
function loadMainLetters () {
  ...
  fontLoader.load('resources/fonts/PirulenRg_3d.json', font => {
    let textGeometry = new THREE.TextGeometry(configuration.SiteName, { font: font, size: 4.5,  height: 1, curveSegments: 5 })
    textGeometry.center()
...
function windowWheelOrTouch (e) {
  ...
  if (e.deltaY > 0 || (e.touches && e.touches[0].pageY < touchStartPosition)) {
    if (sceneMovedAmmount === 3) return
    sceneMovedAmmount++
    sceneMovedAmmount = Math.min(sceneMovedAmmount, 3)
    moveScene()
